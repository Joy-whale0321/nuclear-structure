...
for events_i in range(nevents):
    # 从高斯分布中生成样本
    random_nucleon_size = 32
    nucleon_r     = np.random.normal(loc=mu, scale=sigma, size=random_nucleon_size)
    nucleon_phi   = np.random.uniform(0, 2 * np.pi, size=random_nucleon_size)
    cos_theta = np.random.uniform(-1, 1, size=random_nucleon_size)
    nucleon_theta = np.arccos(cos_theta)

    nucleon_x = nucleon_r * np.sin(nucleon_theta) * np.cos(nucleon_phi)
    nucleon_y = nucleon_r * np.sin(nucleon_theta) * np.sin(nucleon_phi)
    nucleon_z = nucleon_r * np.cos(nucleon_theta)

    # alpha cluster center square
